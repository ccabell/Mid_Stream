...
import type { RunOutputs, V2Pass1Output, V2Pass2Output } from 'apiServices/types';
import { detectExtractionVersion } from './versionDetect';
import { intentLevelToPercentage, intentLevelToLabel, computeOfferingValueMetrics } from './normalize';
...
function buildOpportunitiesCard(p1: V2Pass1Output): OpportunitiesCard {
  ...
  const opportunitiesSummary =
    items.length === 0
      ? null
      : items.length === 1
        ? `1 opportunity: ${items[0]?.name ?? 'Unknown'} — patient interested but not yet booked.`
